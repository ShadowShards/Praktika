...
unsigned int Generation::GenerationAge(int lowAge,int upAge)
{
    std::random_device random_device; // Источник энтропии.
       std::mt19937 generator(chrono::system_clock::now().time_since_epoch().count()); // Генератор случайных чисел.
       // (Здесь берется одно инициализирующее значение, можно брать больше)

       std::uniform_int_distribution<> distribution(18, 99); // Равномерное распределение [10, 20]
...
vector<Person> Generation::GetPersonList(int quantity)
{
    ...
    for (int i=0;i<quantity;i++)
    {
        ...
        person.SetAge(this->GenerationAge(this->upAge,this->lowAge));
